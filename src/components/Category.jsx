...
function Category(categaoryVDstatus) {
...
const videoDrop= async (e,categoryDetails)=>{
 ...
 /* categoryDetails.allVideos.push(videoDetails)
  ...
  if(categoryDetails.allVideos.find((item)=>item.id==videoDetails.id)){
         toast.warning                                                                                                ('Video already in the smae category')
  }
  else{
    categoryDetails.allVideos.push(videoDetails)
    console.log(categoryDetails);
...
const videoDrag=(e, videoDetails,categoryDetails)=>{
  ...
  useEffect(() => {
    getCategory()
  }, [categoryStatus,deletestatus,categoryUpdateStatus,categaoryVDstatus])
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <div className='p-4 rounded border-dark border'>
            <input type="text" placeholder='Enter Category name' className='form-control' onChange={(e) => setCategoryName(e.target.value)} /></div>
        </Modal.Body>
        <Modal.Footer>
